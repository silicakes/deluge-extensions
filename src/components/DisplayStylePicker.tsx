...
export function DisplayStylePicker({ compact = false }: DisplayStylePickerProps) {
  const settings = useSignal(displaySettings.value);
    ...
    const loadSettings = () => {
      try {
        ...
        if (savedSettings) {
          ...
          // Update local signal for UI rendering
          settings.value = displaySettings.value;
        }
      } catch (error) {
  ...
  // Apply updates to the global signal
  const apply = (patch: Partial<DisplaySettings>) => {
    const newSettings = { ...displaySettings.value, ...patch };
  ...
  const saveSettings = useDebouncedCallback(() => {
    try {
      localStorage.setItem('DExDisplaySettings', JSON.stringify(displaySettings.value));
    } catch (error) {
  ...
  if (compact && !expanded.value) {
    return (
      <Button onClick={toggleExpanded} title="Display Style Settings">
        <span role="img" aria-label="Display Settings">🖌️</span>
      </Button>
    );
  }
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-2">
            <input 
              ...
              onInput={(e) => apply({ pixelWidth: clamp(+(e.target as HTMLInputElement).value) })}
            ...
            <input 
              ...
              onInput={(e) => apply({ pixelHeight: clamp(+(e.target as HTMLInputElement).value) })}
              className="w-16 px-2 py-1 border rounded"
            />
            <Button onClick={increaseSize} className="ml-2">＋</Button>
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-2">
            ...
            <input 
              ...
              onInput={(e) => apply({ foregroundColor: (e.target as HTMLInputElement).value })}
        ...
        <div className="flex items-center">
          <input 
            ...
            onChange={(e) => apply({ use7SegCustomColors: (e.target as HTMLInputElement).checked })}
            className="mr-2"
          />
          <label htmlFor="use7SegCustomColors" className="text-sm">Use custom colors for 7-segment</label>
        </div>
      </div>
    </div>
  );
