...
export function MidiDeviceSelector() {
  ...
  useEffect(() => {
    ...
    if (storedOutId) {
      const outDev =
        outputsSignal.value.find((d) => d.id === storedOutId) || null;
  ...
  return (
    <div className="flex flex-col gap-2 p-4 bg-[var(--color-bg-offset)] rounded-lg border border-[var(--color-border)]">
      <div className="flex items-center justify-between mb-2">
        ...
        <span
          className={`text-sm rounded-full px-2 py-1 ${outputSignal.value ? "bg-green-600 text-white" : "bg-yellow-600 text-white"}`}
        >
          {statusText.value}
        </span>
      </div>
      <div>
        <label htmlFor="midi-input-select" className="mr-2 font-medium">
          MIDI Input:
        </label>
        <select
      ...
      <div>
        <label htmlFor="midi-output-select" className="mr-2 font-medium">
          MIDI Output:
        </label>
        <select
      ...
      <div className="flex items-center">
        ...
        />
        <label htmlFor="midi-auto-connect" className="font-medium">
          Auto-connect Deluge Port 3
        </label>
      </div>
    </div>
  );
